            ...
            <div class="row tm-edit-product-row">
              <div class="col-xl-6 col-lg-6 col-md-12">
                <div class="form-group mb-3">
                  <label for="name">Bog Title </label>
                  <input
                    id="name"
                    name="name"
                    type="text"
                ...
                <div class="form-group mb-3">
                  <label for="description">Bog Description</label>
                  <textarea
                    class="form-control validate"
                    rows="3"
                ...
                <button
                  @click="updateBlog"
                  class="btn btn-primary btnAddBlog btn-block text-uppercase"
                >
                  Upadte Blog Now
                </button>
              </div>
              <div class="col-xl-6 col-lg-6 col-md-12 mx-auto mb-4">
                <div class="tm-product-img-dummy mx-auto">
                  <img ref="imgReview" class="js-previewImage" alt="" />
                  <label
                    v-if="!hasChooseImage"
...
export default {
  ...
  async mounted() {
    ...
    this.blogDescription = data.content;
    this.$refs.imgReview.src = data.image.url;
    this.isLoading = false;
  },
  methods: {
    chooseFile() {
      let reader;
      const preViewImage = document.querySelector(".js-previewImage");
    ...
    async updateBlog() {
      ...
      if (
        ...
        (!file.length && !this.$refs.imgReview.src)
      ) {
        this.error = true;
        this.errorMsg = "Please input Blog Title or Blog Content or Blog Image";
        this.isLoading = false;
        return;
      }
      const formData = new FormData();
      formData.append("blog[title]", this.blogTitle);
      formData.append("blog[content]", this.blogDescription);
      if (file && file[0]) {
        formData.append("blog[image", file[0], file[0].name);
      }
      const url = `https://api-placeholder.herokuapp.com/api/v2/blogs/${this.routerId}`;
      const { data } = await CallAPI.putAPI(url, formData, "OK");
      this.isLoading = false;
      if (data.data) {
        this.blogTitle = "";
        this.blogDescription = "";
