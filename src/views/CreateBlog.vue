...
export default {
  ...
  methods: {
    ...
    async createBlog() {
      this.isLoading = true;
      const file = this.$refs.inputFile.files;
      if (!this.blogTitle || !this.blogDescription || !file.length) {
        this.error = true;
      ...
      const url = "https://api-placeholder.herokuapp.com/api/v2/blogs";
      const { data } = await CallAPI.postAPI(url, formData, "Created");
      this.isLoading = false;
      if (data.data) {
        this.blogTitle = "";
